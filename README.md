...
当前为 **0.1.1 发布候选版**。具体构建和测试结果见 [发布记录](docs/RELEASE.md)。Windows 安装包尚未实机验证；手机安装包尚未实现。
...
- 不连接云服务也能正常使用。
- 正常退出使用列表左下角或托盘的「退出轻笺」。程序会等待各便签完成本地保存。
...
0.1.1 安装包已生成，包含上述新增功能。升级沿用原正式版资料库与同步配置，安装前请导出 JSON 备份并正常退出旧版。安装说明见 [发布记录](docs/RELEASE.md)，上传源码见 [GitHub 发布指南](docs/GITHUB.md)。

### 配置坚果云

1. 在坚果云网页版的「账户信息 → 安全选项 → 第三方应用管理」添加“轻笺”，生成应用密码。
2. 打开轻笺「设置与同步」，填写：
...
这会打开「轻笺 · 开发版」，直接运行真实桌面窗口，**不需要打包或安装**。修改 Svelte / CSS / TypeScript 会热更新；修改 Rust 会自动重编译并重启。首次启动需编译，后续为增量构建。在启动终端按 `Ctrl+C` 停止。界面热更新可能重建编辑器，调试时使用测试便签即可。
...
安装包输出到 `target/release/bundle/`。GitHub Actions 在推送 main 时检查并构建 Windows、Linux 安装包；推送匹配版本的 `v*` 标签时，还会创建附带安装包的预发布草稿，需在 Releases 页面确认发布，不会自动公开草稿。
